...
let main = list[0];
const file = require("./lib/our_file.js")
...
let name_property = [{
  ...
  box2.position.y = gui_property.box_y;
  box2.position.x = gui_property.box_x;
  box2.scale.y = 2;
  ...
  scene.add(box2);
   ...
   for(let x in gui_property){
    main.objectGUI({
     ...
     box2.position.y = input.box_y / 100.0;
     box2.position.x = input.box_x / 100.0;
     }
     });
  }
...
window.onresize=()=>{
  ...
  camera.aspect =(window.innerWidth - objectRender.width - panel.leftPanel.width)/(window.innerHeight - objectRender.height);
